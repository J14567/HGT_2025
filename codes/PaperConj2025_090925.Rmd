...
###### [Last revised by:]{.underline} Juliette Bellengier 09/09/2025
...
ggplot(F2_guide1, aes(x = genotype, y = CFUdonnor, fill = guide)) + 
  labs(y = "log(CFU Hfr/pNTM3 donor)") +
...
tiff("Figures/Figure4C.tiff", width = 4.5, height = 4.5, units = "in", res = 500)

ggplot(F4D, aes(x = Recipient, y = CFU_rec)) +
  labs(x = "Recipient", y = "Fraction of galK+ recombinants \n (Log Scale)") + 
...
#Variables qui vont stocker les régions avant et après les régions recombinées
...
RecTotalPlus$sdL=0
RecTotalPlus$sdR=0
#Regarder les biais locaux.... prendre la taille dans les 50kb et faire un Z score. 
scale=50000
for (j in c(1:nrow(RecTotalPlus)))
{
  ...
  #test: tirer au hasard et faire un Z score
...
HS_fragments <- read.csv("Step2/19june/HS_K12/RecombinedFragmentsStats.txt", sep = " ", na.strings = c("NA"))
g536_fragments <- read.csv("Step2/19june/536_K12/RecombinedFragmentsStats0509.txt", sep = " ", na.strings = c("NA"))
...
split_circular_intervals <- function(RecInd, chromosome_length) {
  ...
  for (i in 1:nrow(RecInd)) {
    ...
    if (start <= end) {
      ...
      # Première partie : 0 -> end
      row1 <- row
      row1$Start <- 0
      # Deuxième partie : start -> chromosome_length
      row2 <- row
...
write.table(file="Positions_Fragments2606_CP",Recs,sep=",", quote=FALSE)
...
resultat <- resultat%>%
  group_by(recombinant_num) %>%
  summarise(Total_Longueur = sum(Length)) %>% ungroup()
resultat$Total_Longueur <- as.numeric(resultat$Total_Longueur)

resultat <- resultat %>%
  mutate(recombinant_num = case_when(
    recombinant_num >= 9 & recombinant_num < 28 ~ "REL606", 
    recombinant_num > 28 & recombinant_num <= 47 ~ "HS", 
    recombinant_num > 47 ~ "536",                           
    TRUE ~ as.character(recombinant_num)
  ))

#resultat$recombinant <- factor(resultat$recombinant, levels = c("REL606", "HS", "536"))
ggplot(resultat, aes(x = log10(Total_Longueur), fill = recombinant_num)) +
  ...
  scale_fill_manual(
    ...
    name = "Recipient" 
  ) +  facet_wrap(~recombinant_num, scales = "fixed")+
  theme(
    legend.position = "none", 
...
p2 <- ggplot(resultat, aes(x = log10(Total_Longueur), fill = recombinant_num)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 17, color = "black", linewidth = 0.2) +
  labs(x = "Log10 (Total Length)", y = "Count") + 
  theme_minimal() +
  scale_fill_manual(
  ...
  coord_fixed(ratio = 1)  +
  scale_x_continuous(limits = c(3, 6.1))
tiff("Figures/Figure5_CDE.tiff", width = 13.5, height = 3.5, units = "in", res = 600)
...
tiff("Figures/Figure6D.tiff", width = 4.5, height = 4.5, units = "in", res = 500)
...
FS1$PLASMID <- factor(FS1$PLASMID, levels = c("control", "cas9"))

tiff("Figures/Suppplement1.tiff", width = 4.5, height = 4.5, units = "in", res = 500)
ggplot(FS1, aes(x = GENOTYPE, y = CFUdonnor, fill = GUIDE)) + 
  ...
  scale_fill_manual(values = c("sacB" = "darkgrey", "sgrna" = "orange"))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  theme_bw() +  
  theme(
...
FragmentPosions <- read.csv("Positions_Fragments0908",header=TRUE, sep=",")
Recs <- read.csv("Positions_Fragments2606_CP",header=TRUE, sep=",")
...
#Function for ploting the recombined fragments for each recombinant by method -one plot per recipient 
...
dev.off()
######## Code pour trouver les régions uniques qui ne s'overlap pas via les deux méthodes
get_unique_regions <- function(df1, df2) {
  ...
  for (rec in unique(df1$recombinant_aligned)) {
    ...
    # Fragments HMM sans chevauchement dans CP
    ...
    # Fragments CP sans chevauchement dans HMM
...
```{r}
coverage_table <- read_csv("coverage_table.csv")
coverage_table175 <- read_csv("coverage_table175.csv")
coverage_tableRELHS <- read_csv("coverage_tableRELHS.csv")

RecombinedStats175 <- read.table("bases175/RecombinedFragmentsStatsTotal.txt", sep = "")
RecombinedStats <- read.table("bases/RecombinedFragmentsStatsTotal.txt", sep = "")
RecombinedStatsREL <- read.table("bases/RecombinedFragmentsStatsTotalREL.txt", sep = "")
RecombinedStatsHS <- read.table("bases/RecombinedFragmentsStatsTotalHS.txt", sep = "")
